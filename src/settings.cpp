...
static void BuildDefs() {
    ...
    s_sliders = {
        ...
        { L"Oil fluorescence (lamp)",           0,     1,    0.05f, 2, &c.acid.oilFluor,      nullptr, L"liquid_acid", L"oil_fluor", false, nullptr, 3, L"The lamp makes the oil glow in its own saturated colour, strongest at the film's thin edge and near the lamp, with HDR headroom on that edge band. Added after Film level, so a dimmed film + fluorescence = dark sheet, glowing edges. 0 = today (brief BL)" },
        { L"Fluorescence reach (screen h)",     0.3f,  2,    0.05f, 2, &c.acid.oilFluorReach, nullptr, L"liquid_acid", L"oil_fluor_reach", false, nullptr, 3, L"How far from the lamp the oil still fluoresces, in screen heights (Gaussian falloff). Low = only the lamp side glows, high = the whole frame. Inert while Oil fluorescence is 0" },
        ...
        { L"Dye follows the lamp",              0,     1,   0.05f, 2, &c.acid.dyeLampFollow, nullptr, L"liquid_acid", L"dye_lamp_follow", false, nullptr, 3, L"1 = today: the mass dye is lit by the lamp ramp and brightest at its thin edge. 0 = the dye ignores the lamp and the edge profile and sits at its core level everywhere -- a flat, dim ambient colour the lamp never lifts (brief BL). Inert while Dye brightness is 0" },
        { L"Dark saturation (punch)",           0,     1,   0.05f, 2, &c.acid.darkSat,      nullptr, L"liquid_acid", L"dark_sat", false, nullptr, 3, L"Darker must mean MORE saturated: raises the chroma of the film, the masses and the droplets as their own level falls (k = 1 + value x (1 - level)), keeping their luminance exactly, clipped only at the gamut edge. 0 = today; inert on an element at level 1 (brief BP)" },
        { L"Dye brightness spread",             0,  0.50f, 0.02f, 2, &c.acid.dyeLumVary,   nullptr, L"liquid_acid", L"dye_lum_vary", false, nullptr, 3, L"So the masses are not all one wax: the dye level becomes Dye brightness x (1 +- this). A mass a hole carved keeps one shade for its life; a big gap between oil blobs gets a slow gradient (about a quarter screen across) that rises with it. Subtle: 0.5 moves the mass level ~+-20%. 0 = today. Masses only; inert while Dye brightness is 0 (brief AG-b)" },
        { L"Dye hue spread (deg)",              0,    60,    1,     0, &c.acid.dyeHueVary,   nullptr, L"liquid_acid", L"dye_hue_vary", false, nullptr, 3, L"Hue spread: the dye sits at Dye hue +- up to this many degrees, per carved mass or as a slow gradient across a big gap (rises with it, no flicker). 25 = a hint, 50 = clearly two-tone; past ~60 it turns orange/teal mud (sheet). 0 = today. Masses only; inert while Dye brightness is 0 (brief AG-b)" },
        { L"Dye hue with thickness (deg)",    -90,    90,    1,     0, &c.acid.dyeThickHue,  nullptr, L"liquid_acid", L"dye_thick_hue", false, nullptr, 3, L"Hue turns with depth into a mass, like real dye density: the thin edge keeps Dye hue and the thick core reaches Dye hue + this. Small masses stay edge-coloured, big ones get a core. 0 = today; inert while Dye brightness is 0 (brief AG-b)" },
        { L"Dye core density",               0.42f,    1,   0.02f, 2, &c.acid.dyeCore,      nullptr, L"liquid_acid", L"dye_core", false, nullptr, 3, L"How bright the thick core of a mass is next to its thin rim. 0.42 = today (the rim is the brightest part, the core ~42% of it: a lit wash); 1 = an even, opaque fill that reads as dense pigment. Pulls the mean level up with it, so lower Dye brightness to hold it. Less effect with Dye smoke up; with Dye follows the lamp at 1 the lamp ramp stays. Inert while Dye brightness is 0 (brief BR)" },
        { L"Lamp grey (far corner)",            0,     1,   0.05f, 2, &c.acid.lampGrey,     nullptr, L"liquid_acid", L"lamp_grey", false, nullptr, 3, L"As if the dye far from the lamp gets less light: the corner diagonally opposite the lamp is desaturated (1 = 60% at the corner, fading to none over Lamp grey size), keeping its luminance, so it greys without darkening. One corner at a time; when the lamp moves to the other side the region slides along the frame edge to the new far corner over ~90 s. Never the centre. 0 = off (brief BU)" },
        { L"Lamp grey size (screen h)",      0.25f, 0.60f,  0.05f, 2, &c.acid.lampGreySize, nullptr, L"liquid_acid", L"lamp_grey_size", false, nullptr, 3, L"Radius of the grey corner region in screen heights: full strength over the inner third, gone at this radius. 0.4 = about a rule-of-thirds corner cell. Inert while Lamp grey is 0 (brief BU)" },
        { L"Lamp grey cool",                    0,     1,   0.05f, 2, &c.acid.lampGreyCool, nullptr, L"liquid_acid", L"lamp_grey_cool", false, nullptr, 3, L"How much of the grey is also a cool white balance (luminance kept). 0 = neutral grey; 1 = a clear blue-grey at the corner. Inert while Lamp grey is 0 (brief BU)" },
        { L"Shadow tone (split tone)",          0,     1,   0.05f, 2, &c.acid.shadowTone,   nullptr, L"liquid_acid", L"shadow_tone", false, nullptr, 3, L"Tints the black masses with the complement of the main film hue (it turns with Hue rotation, always opposite), lifting black by up to Shadow tone lift. Film, lit dye and rims untouched. Switches itself on and off (50% of the time) on its own slow clock, out of step with the hue. Gives up true black on the masses. 0 = off (brief BU)" },
        { L"Shadow tone lift",                  0, 0.15f,   0.01f, 2, &c.acid.shadowToneLift, nullptr, L"liquid_acid", L"shadow_tone_lift", false, nullptr, 3, L"How far black is lifted at full tone, fraction of SDR white (sRGB-encoded). 0.06 = a deep tint, 0.12 = clearly coloured bodies. Inert while Shadow tone is 0 (brief BU)" },
        { L"Shadow tone saturation",            0,     1,   0.05f, 2, &c.acid.shadowToneSat, nullptr, L"liquid_acid", L"shadow_tone_sat", false, nullptr, 3, L"Saturation of the shadow tint. 0 = a neutral grey lift; 0.8 = the complement, rich. Inert while Shadow tone is 0 (brief BU)" },
        { L"Shadow tone period (s)",            0,  3600,   25,    0, &c.acid.shadowTonePeriod, nullptr, L"liquid_acid", L"shadow_tone_period", false, nullptr, 3, L"One on+off cycle of the shadow tone, seconds (on for half). 2225 = the hue period / golden ratio, so the tint never meets the same hue twice. 0 = always on. Inert while Shadow tone is 0 (brief BU)" },
        { L"Shadow tone fade (s)",              0,   600,   10,    0, &c.acid.shadowToneFade, nullptr, L"liquid_acid", L"shadow_tone_fade", false, nullptr, 3, L"Length of each on/off crossfade, seconds. Minutes, so the switch is never seen. Inert while Shadow tone is 0 (brief BU)" },
        { L"Shadow tone hue (deg)",            -5,   360,   5,     0, &c.acid.shadowToneHue, nullptr, L"liquid_acid", L"shadow_tone_hue", false, nullptr, 3, L"Leftmost (shown as 'complement') = the tint is the complement of the main film hue and turns with it (default). 0..360 = a FIXED hue in degrees that does not rotate: 275 violet, 235 blue, 185 teal. Inert while Shadow tone is 0 (brief BU/BV)" },
...
static void UpdateSliderLabel(size_t i) {
    ...
    else if (d.key && wcscmp(d.key, L"shadow_tone_hue") == 0 && v < 0.0f)
        swprintf_s(val, L"complement");
    else
