...
static void BuildDefs() {
    ...
    s_sliders = {
        ...
        { L"Dye hue (deg)",                   -180,   180,   5,     0, &c.acid.dyeHue,       nullptr, L"liquid_acid", L"dye_hue", false, nullptr, 3, L"The colour the dark masses take. With Dye follows film on, this is an OFFSET from the film's own hue, so the two turn together; with it off it is an absolute hue. +-180 covers the circle either way" },
        { L"Dye saturation",                    0,     1,   0.05f, 2, &c.acid.dyeSat,       nullptr, L"liquid_acid", L"dye_sat", false, nullptr, 3, L"How coloured the dark masses are. The full 0..1 is useful here because the dye sits at a very low value: even fully saturated it reads as a deep wax, not as a bright fill. 0 = today's neutral black" },
        { L"Dye brightness",                    0,  0.30f, 0.01f, 2, &c.acid.dyeLum,       nullptr, L"liquid_acid", L"dye_lum", false, nullptr, 3, L"How much light the masses let through. Stops above 0.30 because the ramp multiplies this by about 4 toward its bright end, so 0.25 already takes the top stop to full and anything higher only clips. Below ~0.02 it is black. 0 = today's black" },
        ...
        { L"Aberration width (px at 1440p)",    0,     6,   0.1f,  2, &c.post.aberrationPx,  nullptr, L"post", L"aberration_px", false, nullptr, 3, L"How far red and blue are displaced radially at the corners, in pixels at 1440p. The fringe fades out on anything defocused, exactly as a real one does" },
