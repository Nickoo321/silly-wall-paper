...
| Brief BD: the high-ISO speckle in the dark masses. Diagnosed with 4 ablations (Phase 1): the lateral aberration took a first difference of the SHARP source on R and B only, printing the grain as colour (72% of the mass chroma noise); two grains stacked (`[post] film_grain` + the unpaced `[liquid_acid] grain`, 69% of the luma noise); the post lifts (sheen, glint halo, fog, overlay floor) plus per-pixel jitter in the halation/bloom gathers. Fixes: grain `film_grain_chroma` (1 = old additive, 0 = scales the pixel, hue-preserving, calibrated at encoded 0.75 so the open film keeps its level) + `film_grain_density` (0 = old curve, 1 = D-log-E hump); aberration taps from a 1.5 px average (unkeyed) + `aberration_coc` (0 = the approved uniform split, 1 = fades with defocus); sheen + glint halo + `fog_mass_gate` gated by ONE spatial 'deep inside a mass' test (4 taps 40 px out) so the lighter edge band keeps everything; overlay dark floor 0.10 -> 0; halation/bloom jitter to half a tap spacing; `[liquid_acid] grain` dropped where the post pass has its own grain, paced to film_grain_fps. Four keys packed 6/6/6/6 in rg2.w (no root params). acid-rise-12: grain 0, film_grain 0.10, chroma 0, density 1, aberration_coc 1, fog_mass_gate 0.7. Pixel-scale speckle in the shadow: luma -87%, hue -80% (coc 0: -85% / -62%); edge band level -3% (the old grain's own Jensen lift), rim/core 1.43 -> 1.40, width ~5 px unchanged. style=fluid parity md5 `10E36EBF1A74EDFE609065D757300054` held. Sheets `build2/shots/live/bd-sheet-4x.png`, `bd-aber-4x.png`, `bd-range-{density,chroma}.png` | (this commit) |
| Photos: `reference/shots/photos/INDEX.md`, every reference/panel photo tagged with what it shows and why it is kept | afdeed1 |
| `FEATURES.md`: complete key manual generated from `settings.cpp`/`fluid.h`/`main.cpp` (grouped, defaults, live values, help text) | 1b7901b |
| Audit 2026-09-22: acid cbuffer slot table (0 mismatches, 7 free slots), literal budget (tightest chunk 2299 bytes headroom), ring-amplitude ordering (meniscus 0.85 vs. mass_rim/halo 0.098/0.072), EXECUTOR-CARD.md's per-worktree GPU-lock bug (finding 4.1) | 5d271d4 |
| AJ: `boundary_reflect_r`/`boundary_reflect_amt` -- a hue2 seam reaches the droplet rims around it via `oilR`, not just `oilC` at the droplet's own pixel; acid-rise-12 ships 0.33/1.0 | 1ea3611 |
| BG: `--shot` also writes a lossless scRGB-half `.jxr` and a 16-bit Rec.2020/PQ `.png` (true HDR review on the panel via Windows Photos), `tools/jxr-check.ps1` | 4b291c5 |
| Audit 2026-09-22 (2): jxr colour maths verified correct (709->2020, PQ curve, cICP, negatives survive), two failure modes flagged (partial files on failure, silent unmarked PQ png), 43 MB/shot into OneDrive, laP31 collision with the in-flight dye4 branch found before merge | 5da0e87 |
| BC: the lamp casts shadows -- `shadow_amt`/`shadow_len`/`shadow_soft` + `[post] light_z` (negative = backlit); new float4 `laP32` (cbuffer now 1632 bytes); acid-rise-12 ships 0.40/0.12/0.55/0.35 | c68c0bf |
| AG/AM DONE for real: the dark masses are dyed in the display pass on `inkC` (translucent wax, Beer-Lambert-style falloff with a 0.42 floor) after two failed ink-ramp attempts (dead code under `ink_mode=water`, traced end to end); `dye_hue`/`dye_sat`/`dye_lum`/`dye_hue_follow`; acid-rise-12 ships hue 285 / sat 0.8 / lum 0.30 / follow 0 | c5e78d3 |
| BD follow-up: the mass-edge lighter band measured and held separately from the interior (level -3.3%, width unchanged, a user-flagged keeper, not part of the noise problem); `aberration_coc` set back to 0 in acid-rise-12 (1 had been drawing the approved crisp out-of-focus droplet outlines) | 53c9252, b7dc031 |
| meniscus_film_mix (default 0, `laP16.z`): A/B revealed the meniscus and `rim_dark` paint NOTHING in acid-rise-12 today (the `meniscus_from_ink` gate is 0 everywhere on this preset's black-ink surround); the key both recolours the halo from the film and opens the gate | 86d1fd3 |
| Diagrams: camera/DOF/tilt rig geometry to scale from the live preset (script + SVG + PNG) | d921886 |
| Audit 2026-09-22 (3): slot table clean (0 mismatches, 1632 bytes, C++/HLSL order+size agree), `rg2.w` packing verified exact over 625 edge cases, one literal 858 bytes from the cap, ring table corrected (meniscus/rim_dark/swarm_lens all dead by gate or by code in acid-rise-12), lid explained as a post-pass layer not a ring term, gravity putF + 3 out-of-range sliders flagged | 1bdf5ad |
| `tools/preset-identity.ps1`: acid-side equivalent of the fluid parity check -- renders a preset list headless, md5's the PNGs, saves/compares a baseline; fluid parity always checked first and separately | 94e1f2b |
| `tools/gpu-lock.ps1`: lock always resolves to the MAIN repo's absolute path regardless of which worktree dot-sources it (audit finding 4.1: a worktree-derived path had been locking a private, gitignored file that excluded nothing) | ddf8500 |
| EXECUTOR-CARD.md hygiene: the 2026-09-15 OLED-grey incident line restored next to the relaxed lock policy, the unactionable `SendUserFile` instruction removed (sheets now copied to `build2\shots\live\`), `tools/build-wt.cmd` tracked instead of reinvented per worktree, literal headroom noted | 5c07720 |
| Booking (Sonnet): the ten `[liquid_acid]`/five `[post]` keys landed 2026-09-22 added at their `fluid.h` defaults, with acid-rise-12's own comment blocks, to the same 33 acid files `f840685`/`6e4a81c`/`2011d8a` used | b44991e |
| Booking (Sonnet) hygiene: `dye_hue`/`hue_rotate_period`/`ink_soft` slider ranges widened to contain their live values; `WriteConfigToIni` now writes back `gravity`/`gravity_pow`/`gravity_blur`; AGENTS.md's bloom/GPU-lock wording corrected; `preset-identity.ps1` at `--shot-yield 8` with a bounded, non-throwing 20-min PNG-wait timeout | 43dfbf8 |
