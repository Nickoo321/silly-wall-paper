...
// per pixel and renders it POSTERIZED: flat saturated interiors, a thin
// bright rim exactly at the field == 1 boundary, a dark meniscus band just
// inside, and hash-noise speckle concentrated near interfaces. A tiny CPU
// sim drifts the blobs. No bloom/glow of any kind; SDR sRGB only.
#pragma once
...
struct AcidConfig {
    int   palette  = 0;      // 0 = Coral (coral / teal / amber), 1 = Royal (purple / red veins)
    float fpsLimit = 60.0f;
};

class AcidRenderer {
public:
    void Init(HWND hwnd, int width, int height, const AcidConfig& cfg);
    void Shutdown();
    void Reattach(HWND hwnd);       // swapchain-only rebuild (Explorer restart)
    void Frame(float dt);
    bool PresentBroken() const { return m_presentBroken; }
    void SetPalette(int p) { m_cfg.palette = p & 1; SeedBlobs(); }
    int  Palette() const   { return m_cfg.palette; }
    AcidConfig& Config()   { return m_cfg; }
