...
| `[liquid_acid] hue_anchor_weight` | Hue rotation: linger at anchors | 0..1, step 0.05 | 0.0 |  | Warps the hue_rotate_period clock (CPU): the rotation lingers at the proven anchors (magenta 325 x1.0, blue 215 x0.6, red 355 x0.5, violet 275 x0.4; gaussians of 20 deg, density 1+4*w*sum) and glides faster through lime/mustard/olive. W(0)=0, W(1)=360, seamless. 0 = the old linear clock exactly. Brief BV |
| `[liquid_acid] palette_start_hue` |   start hue (deg) | -2..359, step 1 | -1 |  | Where the palette clock STARTS, CPU only (no slot): at renderer init and at each cycle stage entry at black (never on the no-black scheme change) the ANIM_PALETTE accumulator is set so the palette's absolute hue (effOil[0], the upload's own palette) is this hue -- sweep off: exact inverse of the anchor warp (PalettePhaseForHue); sweep on (monotone-post-0924): a search over 8192 clock phases + refine. -1 = today (starts on oil_color_1's own hue, orange on monotone). -2 = RANDOM: a start hue drawn from the anchor density at weight 1 (magenta 325 / blue 215 / red 355 / violet 275 hold ~58% of draws), own splitmix RNG (never rand()), seeded from the wall clock, or from --seed headless. Measured: palette hue at the first frame = the set hue +-0.02 deg on all 16 Scheme presets; the rendered film's HSV mode is within 5 deg for magenta/blue/teal, 9-12 deg off for violet/red (the display chain's own shift: the old orange start is 11 deg off the same way). Schemes ship their anchor (325 / 215 / 275 / 190 / 10 / 230); monotone-post-0924 ships -2 (user). Brief BW |
...
| `[liquid_acid] film_hue2_cover` |   second hue coverage | 0..0.25, step 0.01 | 0.0 |  | CPU bias on the hue2 mix field's target, tgt' = saturate(tgt + cover), applied only where patch material is made (below the edge); no slot, no shader change. Share of visible mix cells > 0.62 at 150 s (Magenta/Mint, seed 1234): 0 -> 10%, 0.04 -> 18%, 0.08 -> 30%, 0.12 -> 38%, 0.16 -> 45%, 0.25 -> 70%; it also shrinks the hue3 low end (< 0.32: 12% -> 1%). Logged per run by the [cover] shot-log line (--cover-sweep a,b,.. adds CPU-only shadow fields). Schemes: 0.08 at film_hue2_scale 0.25 on the Magenta/Mint, Magenta/Cyan, Blue/Coral pairs; 0.04 (+ film_hue3_share 1) elsewhere. Brief BW multi-seed swing (--cover-sweep b@scale --cover-seeds 12, Magenta/Mint flow, 12 noise seeds x 460 s): scale 0.40/cover 0.08 mean 33% P95 64% max 91%; scale 0.25/cover 0.08 mean 36% P95 58% max 82% (0-180 s: 37% / 59% / 65%), still ~3 patches with the biggest ~70% of the colour; 0.25/0.10 mean 43% P95 65%. FINAL-CYCLE C + BW |
| `[liquid_acid] film_hue2_seam` |   seam width | 0.25..1, step 0.05 | 1.0 |  | Width of the k2 (and k3) band around its centre as a factor of today's half-width (0.06 around 0.62; 0.14 around 0.32 - h3d), laP35.w; exactly 1 takes the literal smoothstep(0.56, 0.68) / (0.18, 0.46) under a [branch] (bit-identical), and the AJ boundary reflection reads the same narrowed bands (k3s, k2m). Measured, Magenta/Mint at 1440p: median seam crossing 118 px (1) -> 52 (0.5) -> 37 (0.33); green-band pixels x0.51 / x0.37, orange/yellow x0.57 / x0.43; seam midpoint OKLab h 92 C 0.12 held. Blue/Coral: violet+magenta+red band 16% -> 7.7% of the frame at 0.33. key-effect MAD 4.7 (0.33 vs 1). Schemes ship 0.33. Brief BW |
...
| `[liquid_acid] film_equal_load` | Film equal load | 0..1, step 0.05 | 0.0 |  | Per pixel, BASE film only (never a patch): dims a film brighter than the same S/V at magenta (325 deg) to that luminance in linear light, so yellow/green/cyan films load the panel like magenta. Dims only. 0 = off. Brief BV |
| `[liquid_acid] film_equal_load_patches` |   equal load on patches | 0..1, step 0.05 | 0.0 |  | The film_equal_load target (the same S/V at magenta 325, linear light, dims only) on the PATCH CORES: weight smoothstep(0.8, 1, k2) + smoothstep(0.8, 1, k3), never raw k (that would dim the seam's yellow-orange to olive); laP38.x; 0 = the old weight under a [branch] (bit-identical). Measured, Magenta/Mint 20 s: frame mean luminance -10% (0.5) / -17% (0.75) / -24% (1); mint cores OKLab L 0.72 -> 0.62 (0.75); seam pixels (HSV 25-80) C 0.132 and h 89 unchanged at every value; Square+ gold / Bright Triad lime cores unchanged at 0.25-0.5 (L 0.64 / 0.63). A thin undimmed lime ring (k 0.6-0.8) stays between seam and core -- narrower with film_hue2_seam 0.33. key-effect MAD 6.8 (0.75 vs 0). Schemes: 0.75 on cool members, 0.25 on patterns with a yellow/gold/amber/orange member. Brief BW |
...
| `[liquid_acid] shadow_tone_period` | Shadow tone period (s) | 0..3600, step 25 | 2225 | 2225 | (brief BU) On+off cycle; 0 = always on. 2225 = 3600 / golden ratio: gear log over 3 h puts the edges at hue phases 93/217/316/79/178/302/41/164/263 deg, first repeat (same edge within 5 deg) after 21.3 h; the 3:5 option 2160 repeats exactly after 3.25 h. Default: fully on 0-872 s, fades out 872-992, off to 2105, back on by 2225. BU-b: the clock's origin moves to NOW when the tone keys (shadow_tone / highlight_tone_amt / highlight_tone_desat) go from all-zero to nonzero, or the acid look returns after > 1 s away, so switching it on always starts the ON half (series, period 60 / fade 5: toggled at 38 s -> gate 1 at 40..56, fade 58-63, off; constant key -> off 40..52, fades in 55-60). Never moves while the keys stay put |
...
| `[liquid_acid] highlight_tone_amt` | Highlight tone (split tone) | 0..1, step 0.05 | 0.0 | 0 | (brief BU-b) Lightroom split-tone HIGHLIGHTS: the lit FILM takes a white-balance multiply toward the tint at constant DISPLAYED luminance (BubLum = the gamut stretch + its negative clamp, measured +33% of the magenta film's Y; a 709-weight hold read -3.4..-22% mean_lum), capped so no channel exceeds the pixel's own max, put back exactly (scale down / bisection toward the flat grey at that max), then the exact inverse of post_lift/post_chroma. Weight alpha x smoothstep(split-0.25, split+0.05, encoded displayed Y). Gated by BU's clock. PROOF (monotone-post-0924 + shadows 171/1, 1280x720 t=40, jxr): film OKLab hue 7.2 -> 9.0 / 13.7 / 18.7 deg at amt .3/.6/1 (hue 45 sat 1), mean_lum -0.6% at amt 1, p99.9 Y +0.2%, p99.9 peak channel 433 -> 417 nits; frame MAD vs shadows-only 7.8 / 11.8 / 16.1; key-effect MAD 20.5 (0.6 vs 0). What adds: 0.3 barely warm, 1 = coral; cannot reach gold on this film (red is at the clip) |
| `[liquid_acid] highlight_tone_hue` | Highlight tone hue (deg) | -5..360, step 5 ("complement" below 0) | -1 | -1 | (brief BU-b) HSV degrees; -1 = the OKLab complement of the RESOLVED shadow hue (solved on the CPU like BU; with shadow_tone_hue -1 that is the film's own hue). key-effect MAD 2.4 (45 vs -1 with shadows 171): on a saturated magenta film the Y-held multiply only swings the film between its red neighbours, so hue moves little |
| `[liquid_acid] highlight_tone_sat` | Highlight tone saturation | 0..1, step 0.05 | 0.50 | 0.5 | (brief BU-b) Tint saturation; key-effect MAD 8.0 (0.8 vs 0.5). OVERLAPS amt: amt 1 x sat .5 -> hue 15.1 deg, amt .6 x sat 1 -> 13.7 (kept for Lightroom parity) |
| `[liquid_acid] highlight_tone_desat` | Highlight film desaturation | 0..0.8, step 0.05 | 0.0 | 0 | (brief BU-b) Film chroma toward grey at the displayed luminance, same weight and gate; folded into the laP39 magnitude (desat = dot(U,Y709) - 1). Film OKLab C 0.253 -> 0.198 / 0.146 / 0.088 at .25/.5/.75 (bu-lr-1 film: 0.248 -> 0.135 = desat ~0.55); mean_lum +0.2 / +1.1 / +0.3%, p99.9 Y +2..5%, peak channel 433 -> 346 / 265 / 195 nits (less ABL). Frame MAD vs shadows-only 20.7 / 31.1 / 42.1 |
| `[liquid_acid] tone_balance` | Tone balance | -1..1, step 0.05 | 0.0 | 0 | (brief BU-b) Lightroom Balance: split = 0.5 - 0.25 b (encoded); highlight band moves with it, shadow band (0.02, 0.15) x (1 - 0.5 b), in its own branch so b = 0 keeps BU's smoothstep bit for bit. At amt 1: film hue 12.6 / 18.7 / 20.1 deg at b -0.5/0/+0.5 (the lit film is fully weighted from +0.5 up) |
...
**434 keys total** (ini-backed; excludes the registry-only "Start with Windows" row; +3 from brief BW), **362 with sliders**, **72 read-only** (no control in the settings window, main.cpp-only), **37 inert or unverified** in the current live preset (the original 10 above plus 29 new from brief BJ's 30-key sweep, one of which — `swarm_lens` — was already counted; brief BW moved `film_hairs` and `film_leak` to time-gated).
